...
public abstract class LifecycleMBeanBase extends LifecycleBase
    ...
    protected void initInternal() throws LifecycleException {
        ...
        // preRegister().
    	/**
    	 * myOpinion **MBean的生成入口
    	 * 			1、mserver是统一的一个
    	 * 			2、register（）是核心，
    	 *               1）传递this对象作为参数，方法会根据this对象类型获得ManagedBean,并将自己作为实际管理对象，然后使用注册
    	 *               2）registerComponent（）中调用的findManagedBean（）方法是核心方法，
    	 *               3）默认情况下在被管理类的包中查找mbeans-descriptors.xml，使用digester将xml装换成对象树
    	 * myOpinion **MBean生成流程：
